...
static time_t ngx_stream_lua_mgr_timer(void *data);
...
static time_t
ngx_stream_lua_mgr_timer(void *data)
{
    ngx_stream_lua_mgr_timer_t  *timer = (ngx_stream_lua_mgr_timer_t *)data;
    ngx_stream_lua_main_conf_t  *lmcf = timer->lmcf;
    ngx_msec_t                   next_secs = (ngx_msec_t) timer->timer_secs;

    int                          nargs, co_ref;
    u_char                      *p = NULL;
    ngx_event_t                 *ev = NULL;
    ngx_stream_session_t        *s = NULL;
    ngx_connection_t            *saved_c = NULL, *c = NULL;
    ngx_stream_lua_ctx_t        *ctx = NULL;
    ngx_stream_lua_timer_ctx_t  *tctx = NULL;
    u_char                      *script_path = NULL;
    ngx_stream_lua_cleanup_t    *cln = NULL;
    ngx_pool_t                  *pool = NULL;

    lua_State* L = lmcf->lua, *co = NULL; /* the main thread */
    ...
    if (lmcf->watcher == NULL) {
        ...
        if (lmcf->watcher == NULL) {
            ngx_log_error(NGX_LOG_ALERT, ngx_cycle->log, 0,
                          "stream lua mgr timer alloc no memory");
            return next_secs;
        }
    ...
    // p = ngx_alloc(sizeof(ngx_stream_lua_timer_ctx_t), timer->log);
    // if (p == NULL) {
    //     goto nomem;
    // }
    ...
    // tctx->co_ref = co_ref;
    // tctx->co = co;
    // tctx->main_conf = timer->main_conf;
    // tctx->srv_conf = timer->srv_conf;
    // tctx->lmcf = lmcf;
    ...
    c = ngx_stream_lua_create_fake_connection(pool);
    if (c == NULL) {
        goto failed;
    }
    ...
    c->listening = timer->listening;
    c->addr_text = timer->client_addr_text;
    ...
    s->main_conf = timer->main_conf;
    s->srv_conf = timer->srv_conf;
...
#   endif
    ...
    ctx = ngx_stream_lua_create_ctx(s);
    if (ctx == NULL || ctx->vm_state == NULL) {
        goto failed;
    }

    // if (ctx->vm_state == NULL {
    //     ctx->vm_state = tctx.vm_state;
    ...
    script_path = ngx_stream_lua_rebase_path(pool,
                                             timer->src_name.data,
                                             timer->src_name.len);
    ...
    ngx_log_debug2(NGX_LOG_DEBUG_STREAM, ngx_cycle->log, 0,
                   "stream lua mgr timer expired (running: %i, max: %i)",
                   lmcf->running_timers, lmcf->max_running_timers);
    ...
    ngx_log_debug1(NGX_LOG_DEBUG_STREAM, s->connection->log, 0,
                   "stream lua creating new mgr timer with delay %M", next_secs);
    ...
    lua_createtable(co, 0, 0);  /* the new globals table */
    /* co stack: new_global_tb */

    lua_createtable(co, 0, 1);  /* co stack: new_global_tb metatable */
    ngx_stream_lua_get_globals_table(co); /* co stack: new_global_tb metatable old_global_tbl */
    lua_setfield(co, -2, "__index");  //metatable.__index = old_global_tbl;  co stack: global_tb metatable
    lua_setmetatable(co, -2); // setmetatable(new_global_tb, {__index = metatable}); co stack: new_global_tb
    ...
    lua_pushlightuserdata(L, &ngx_stream_lua_coroutines_key);
    lua_rawget(L, LUA_REGISTRYINDEX);
    /* L stack: func thread corountines */

    /* L stack: time func [args] thread corountines */

    lua_pushvalue(L, -2);
    /* L stack: func thread corountines thread */
...
failed:
    ...
    if (co_ref && co) {
        lua_pushlightuserdata(co, &ngx_stream_lua_coroutines_key);
    ...
    // if (vm_state) {
    //     ngx_stream_lua_cleanup_vm(vm_state);
    // }
    ...
    ngx_log_error(NGX_LOG_ALERT, ngx_cycle->log, 0,
                      "stream lua: call mgr timer failed");
...
//     if (rc != NGX_OK) {
//         ngx_log_error(NGX_LOG_ALERT, ngx_cycle->log, 0,
//                   "stream lua: process timer in manager process failed %d", rc);
//     }
    ...
    void *conf)
{
    ...
    if (value[1].len == 0) {
        ngx_conf_log_error(NGX_LOG_EMERG, cf, 0,
                           "invalid manager timer lua file name value \"%V\"", &value[1]);
        return NGX_CONF_ERROR;
    }

    timer->src_name.data = value[1].data;
    timer->src_name.len = value[1].len;
    ...
    *p = '\0';
    ...
    timer->path->manager = (ngx_path_manager_pt)ngx_stream_lua_mgr_timer;
    ...
    timer->timer_secs = timer->timer_secs / 1000;
    timer->timer_secs == 0 ? 1 : timer->timer_secs;

    if (ngx_add_path(cf, &timer->path) != NGX_OK) {
        return NGX_CONF_ERROR;
    }
